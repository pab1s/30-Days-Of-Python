...
print(convert_celcius_to_fahrenheit(32))
...
def check_season(month):
    ...
    if month == 'September' or month == 'October' or month == 'November':
        ...
        season = 'You did not enter a month.'
    return season
...
print(calculate_slope(linear_eq))

# Exercise 7
def quadratic_eq(x):
    return x**2 + 2*x + 1

def solve_quadratic_eq(f):
    a, b, c = 1, 2, f(0)
...
print(solve_quadratic_eq(quadratic_eq))
...
print(add_item(['potato', 'tomato', 'mango', 'milk'], 'onion'))
...
def calculate_variance(nums):
    ...
    variance = 0
    for xi in nums:
        variance +=(xi - mean)**2 / len(nums)
    return variance

print(calculate_variance(numbers))

def calculate_std(nums):
    std = calculate_variance(nums)**(1/2)
    return std
...
# Exercise 21
def items_unique_list(lst):
    return len(set(lst)) == len(lst)

print(items_unique_list(['A','B','C']))

# Exercise 22
def same_type_list(lst):
    same_type = True

    for ele in lst: 
        if type(ele) != type(lst[0]):
            same_type = False
            break
...
print(same_type_list(['A','B',0]))

# Exercise 23
def is_valid(ele):
    return str(ele).isidentifier()

print(is_valid('Hi_you'))
